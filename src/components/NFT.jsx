    ...
    <Stack>
        {onModalImg ? 
            <img src={data.image} alt={data.name} onClick={() => setOnModalImg(false)} 
                style={{position: "absolute",top:0, left:0, right:0, bottom:0, margin:"auto" ,zIndex: "1"}}/> 
            : null}
        <Stack sx={{flexDirection:"row", margin: 3}}>
            <Stack sx={{margin: 3, textAlign: "center"}}>
            ...
            <Stack sx={{ width:"100%", alignItems:"center", textAlign:"center", border: 1, margin: 5, padding: 3}}>
                <Stack sx={{flexDirection:"row", margin: 3}}>
                    <Input placeholder="토큰을 전송할 주소" sx={{marginRight: 3, width: 500}}/>
                    <Button variant="contained" sx={{ width:110, float: "right"}}>Send</Button>
                </Stack>
                <Stack sx={{padding: 1.5, borderRadius: 3, fontSize: 45}}>
                    NFT name : {data.name}
                </Stack>
                <Stack sx={{border: 1, padding: 1.5, marginTop:5, borderRadius: 3}}>
